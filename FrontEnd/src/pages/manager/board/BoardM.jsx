...
export const BoardM = () => {
    ...
    const handleButtonClickWrite = () => {
        window.location.href = 'http://localhost:5173/manager/board/write';
    };
                ...
                <table className="board-table" border="1">
                    ...
                    <tbody>
                        {currentPosts.map((post, index) => (
                            <tr key={post.id}>
                                <td id="post-number">
                                    <a href={`/manager/board/view`}>{indexOfFirstPost + index + 1}</a>
                                </td>
                                <td id="post-title">
                                    <a href={`/manager/board/view`}>{post.title}</a>
                                </td>
                                <td id="post-author">
                                    <a href={`/manager/board/view`}>{post.author}</a>
                                </td>
                                <td id="post-date">
                                    <a href={`/manager/board/view`}>{post.date}</a>
                                </td>
                            </tr>
                        ))}
                    </tbody>
